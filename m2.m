function [list] = m2(lattice)
%M1 Do block entropy by length one, return array with info
%   Detailed explanation goes here
numOfCombinations=16;
...
for i=1:lengthOfLattice
    for j=1:lengthOfLattice
        ...
        sweep(1)=lattice(mod(i-1,lengthOfLattice)+1,mod(j+1,lengthOfLattice)+1);
        sweep(2)=lattice(mod(i,lengthOfLattice)+1,mod(j+1,lengthOfLattice)+1);
        sweep(3)=lattice(mod(i,lengthOfLattice)+1,mod(j-1,lengthOfLattice)+1);
        sweep(4)=lattice(mod(i+1,lengthOfLattice)+1,mod(j-1,lengthOfLattice)+1);
